    ...
    <div className="mx-auto max-w-4xl space-y-6 pb-12">
      ...
      <div className="flex flex-col justify-between gap-4 rounded-xl border bg-white p-6 sm:flex-row sm:items-center">
        ...
        <div className="flex flex-wrap gap-2">
          {data.status === 'PENDING' && (
            <Button onClick={() => setStatus('CONFIRMED')} disabled={updateStatus.isPending}>
              Xác nhận
            </Button>
          )}
          {['PENDING', 'CONFIRMED', 'RESCHEDULED'].includes(data.status) && (
            <Button variant="outline" onClick={() => setStatus('CANCELED')} disabled={updateStatus.isPending}>
              Hủy lịch
            </Button>
          )}
          {data.status === 'CONFIRMED' && (
            <Button onClick={() => setStatus('COMPLETED')} disabled={updateStatus.isPending}>
              Hoàn tất
            </Button>
          )}
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <section className="space-y-4 rounded-xl border bg-white p-6">
          ...
          <p className="flex gap-2">
            ...
            {new Intl.DateTimeFormat('vi-VN', { dateStyle: 'full', timeStyle: 'short' }).format(
        ...
        <section className="space-y-4 rounded-xl border bg-white p-6">
          ...
          <p className="flex gap-2">
            ...
            {data.renter.phone ?? 'Chưa có số điện thoại'}
          </p>
