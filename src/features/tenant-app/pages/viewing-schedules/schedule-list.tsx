  ...
  return (
    <div className="mx-auto max-w-[1200px] space-y-6 pb-10">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder="Tìm khách, số điện thoại hoặc phòng"
            className="pl-9"
          />
        </div>
        <Select
      ...
      {isLoading ? (
        ...
        <button type="button" onClick={() => void refetch()} className="w-full py-16 text-center text-red-600">
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y">
              {data.data.map((appointment) => (
                ...
                >
                  <td className="p-4">
                    <div className="font-medium">{appointment.renter.fullName}</div>
                    <div className="text-xs text-slate-500">{appointment.renter.phone ?? appointment.renter.email}</div>
                  </td>
                  <td className="p-4">
                    <div className="font-medium">{appointment.room.title}</div>
                    <div className="text-xs text-slate-500">{appointment.room.roomCode}</div>
                  </td>
                  <td className="p-4">
