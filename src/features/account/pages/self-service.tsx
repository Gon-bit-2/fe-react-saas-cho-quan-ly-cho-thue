...
function itemDescription(item: SelfServiceItem): string | undefined {
  return nestedLabel(item.property)
    ?? nestedLabel(item.room)
...
function itemDate(item: SelfServiceItem): string | undefined {
  const raw = textValue(item.scheduledAt)
    ?? textValue(item.startDate)
    ?? textValue(item.dueDate)
...
export function Component() {
  ...
  const cancelItem = useMutation({
    mutationFn: (id: number) => {
      const endpoint = slug === 'lich-xem-phong'
        ? `/room-viewing-appointments/me/${id}/cancel`
        : `/rental-requests/me/${id}/cancel`
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      {query.isLoading && <div className="rounded-xl border bg-white p-8 text-center text-slate-500">Đang tải dữ liệu...</div>}

      {query.isError && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-5 text-red-700">
          Không thể tải dữ liệu. Vui lòng thử lại sau.
          <button className="ml-2 underline" type="button" onClick={() => void query.refetch()}>Thử lại</button>
        </div>
      )}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {query.data?.data.map((item, index) => {
          ...
          const canCancel = Number.isInteger(itemId) && (
