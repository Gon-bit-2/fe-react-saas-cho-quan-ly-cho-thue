import { useViewHistory } from '@/shared/api/marketplace'
import { RoomCard } from '@/features/marketplace/components/room-card'
import { Link } from 'react-router'

export function Component() {
  const { data: viewHistory, isLoading } = useViewHistory()

  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-headline-md text-text-main">Lịch sử xem phòng</h1>
        <p className="font-body-md text-on-surface-variant mt-1">Danh sách các phòng bạn đã xem gần đây</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-surface-container-lowest border-surface-border rounded-xl border p-12 text-center shadow-sm">
          ...
          <h3 className="font-headline-sm text-text-main mt-4">Chưa có lịch sử</h3>
          <p className="font-body-md text-on-surface-variant mt-2 mb-6">
            Bạn chưa xem phòng nào gần đây. Hãy khám phá ngay nhé!
          </p>
          <Link
            to="/phong"
